...
lidar.data = (C_li @ lidar.data.T).T + t_i_li       # n*3
...
p_est[0] = gt.p[0]          # 位置
v_est[0] = gt.v[0]          # 速度
q_est[0] = Quaternion(euler=gt.r[0]).to_numpy()         # 朝向
...
#     K_k = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T+np.eye(3)*sensor_var)         # 计算增益
#
#     # 3.2 Compute error state
#     delta_x_k = np.dot(K_k,(y_k - p_check)).reshape((9,1))      # 9*3 * 3*1 = 9*1
#     delta_p_k = delta_x_k[:3,0]
#     delta_v_k = delta_x_k[3:6,0]
#     delta_q_k = delta_x_k[6:,0]
#
#     # 3.3 Correct predicted state
#     p_hat = p_check + delta_p_k
#     v_hat = v_check + delta_v_k
#     q_hat = Quaternion(axis_angle=delta_q_k).quat_mult_right(q_check)      # 4*1
#
